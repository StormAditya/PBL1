...
class MassiveSchedulingEnv(gym.Env):
    ...
    def _get_state(self):
        ...
        norm_job = current_job_size / 50.0 
...
def run_sjf_scheduler(jobs, num_machines):
    machines = [0.0] * num_machines
    sorted_jobs = sorted(jobs) # Ascending sort
...
for episode in range(1000):
    ...
    if (episode + 1) % 250 == 0:
        print(f"Training Episode {episode + 1}/1000 Complete...")
...
for bar in bars:
    yval = bar.get_height()
    plt.text(bar.get_x() + bar.get_width()/2, yval + 5, f"{yval:.1f}s", ha='center', va='bottom', fontweight='bold')

plt.ylim(top=max(values) + 50)
plt.tight_layout() 
plt.show()