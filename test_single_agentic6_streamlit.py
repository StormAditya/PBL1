...
st.markdown("""
Compare traditional scheduling algorithms (Random, First-Come-First-Serve, Shortest Job First) 
against a trained Reinforcement Learning (Agentic AI) model.
""")
...
if st.button("Run Algorithm Showdown", type="primary"):
    with st.spinner("Loading environment and model..."):
        ...
        try:
            ...
            st.error(f"ERROR: '{model_path}' not found. Make sure you've run the training script and the file is in the same directory!")
            st.stop()

    with st.spinner(f"Running {num_test_scenarios} Test Scenarios..."):
        # Generate identical test scenarios
        test_scenarios = [[(np.random.randint(10, 101), random.choice([0, 1])) for _ in range(env.num_jobs)] for _ in range(num_test_scenarios)]
    ...
    st.subheader("📊 Final Average Makespans (Lower is Better)")
    ...
    ax.set_title("Real-World Factory Scheduling Showdown", fontsize=14, pad=15)
    ax.set_ylabel("Makespan (Seconds) - Lower is Better", fontsize=12)

    for bar in bars:
        yval = bar.get_height()
        ax.text(bar.get_x() + bar.get_width()/2, yval + (max(values)*0.01), f"{yval:.1f}s", ha='center', va='bottom', fontweight='bold')
