...
# ==========================================
# 1. The Environment (HARD MODE: Heterogeneous & Affinities)
# ==========================================
def get_actual_processing_time(job_size, job_type, machine_idx):
    """Calculates true processing time based on hardware affinities."""
    # Define Machine Hardware Types
    ...
    if job_type == machine_type:
        return (job_size * base_speed) * 0.5  # 2x FASTER: Perfect hardware match!
    else:
        return (job_size * base_speed) * 3.0  # 3x SLOWER: Hardware mismatch bottleneck!
...
class RealWorldSchedulingEnv(gym.Env):
    ...
    def _get_state(self):
        ...
        for i in range(self.lookahead):
            ...
            if idx < self.num_jobs:
                state_jobs.append(self.jobs[idx][0] / 100.0) # Normalize size
                state_jobs.append(float(self.jobs[idx][1]))  # Job type (0.0 or 1.0)
            else:
                state_jobs.extend([0.0, 0.0]) # Pad empty future slots
    ...
    def step(self, action):
        ...
        # Calculate processing time with hardware affinities
        ...
        # Reward AI for keeping total time low and machines balanced
...
# ==========================================
# 2. The Neural Network (Expanded 30-Input Brain)
# ==========================================
class DQN(nn.Module):
...
# ==========================================
# 3. Traditional Baseline Algorithms (Blind to Affinities)
# ==========================================
...
def run_fcfs_scheduler(jobs, num_machines):
    machines = [0.0] * num_machines
    for job_size, job_type in jobs:
        # FCFS blindly picks the machine with the lowest current load
...
def run_sjf_scheduler(jobs, num_machines):
    machines = [0.0] * num_machines
    # SJF sorts purely by base job size, ignoring hardware types
...
# 4. Training the DDQN
# ==========================================
print("Training AI to Master Hardware Affinities...")
...
min_epsilon = 0.05
epsilon_decay = 0.997 
episodes = 3500 # Slightly longer to learn the hardware map
...
# Generate 10 complex test workloads (100 jobs each)
